import torch
import torch.nn as nn
import torch.optim as optim
from torch.utils.data import Dataset, DataLoader

# PyTorch Geometric imports
from torch_geometric.nn import knn_graph, PointTransformerConv
from torch_geometric.nn.dense.linear import Linear
from torch_geometric.utils import to_undirected, remove_self_loops, add_self_loops, softmax
from torch_geometric.data import Data  # <-- Add this line

# ------------------------------
# Data Preprocessing Functions
# ------------------------------

def precompute_knn_inplace(filtered_data, k=10):
    """
    Loops through filtered_data and precomputes the k-NN graph for each sample's 'dep_points'.
    Stores the result in sample['dep_edge_index'].
    
    Args:
        filtered_data: List of dicts, each with:
            'dep_points': Tensor of shape [3, N_dep],
            'uav_points': Tensor of shape [3, N_uav]
        k: Number of neighbors for the k-NN graph.
    """
    for sample in filtered_data:
        dep_points = sample['dep_points']  # shape: [3, N_dep]
        dep_points_t = dep_points.t().contiguous()  # shape: [N_dep, 3]
        
        # Build the k-NN graph with loop=False (i.e. no self-loops)
        edge_index = knn_graph(dep_points_t, k=k, loop=False)
        # Convert to an undirected graph (adds reverse edges)
        edge_index = to_undirected(edge_index, num_nodes=dep_points_t.size(0))
        
        # Store the precomputed edge index in the sample dictionary
        sample['dep_edge_index'] = edge_index


def normalize_pair(dep_points, uav_points):
    """
    Given two point clouds (dep_points and uav_points) each of shape [N, 3],
    compute a common center and scale so that both clouds are normalized in the same coordinate system.
    
    Returns:
        dep_points_norm: Tensor of shape [N_dep, 3] (normalized)
        uav_points_norm: Tensor of shape [N_uav, 3] (normalized)
        center: Tensor of shape [1, 3] representing the centroid of the combined clouds
        scale: Scalar value representing the bounding radius (maximum distance from the center)
    """
    # Combine both point clouds along the first dimension
    combined = torch.cat([dep_points, uav_points], dim=0)  # shape: [N_dep + N_uav, 3]
    center = combined.mean(dim=0, keepdim=True)  # shape: [1, 3]
    combined_centered = combined - center
    scale = combined_centered.norm(dim=1).max()  # single scalar
    scale = scale.clamp_min(1e-9)  # avoid division by zero

    dep_points_norm = (dep_points - center) / scale
    uav_points_norm = (uav_points - center) / scale

    return dep_points_norm, uav_points_norm, center, scale

# ------------------------------
# Dataset and Collation
# ------------------------------

class PointCloudUpsampleDataset(Dataset):
    def __init__(self, data_list):
        """
        Args:
            data_list: List of dicts, each with:
                'dep_points': Tensor of shape [3, N_dep]
                'uav_points': Tensor of shape [3, N_uav]
                'dep_edge_index': Tensor of shape [2, E] (precomputed edges)
        """
        self.data_list = data_list

    def __len__(self):
        return len(self.data_list)

    def __getitem__(self, idx):
        sample = self.data_list[idx]
        dep_points = sample['dep_points'].t().contiguous()  # shape: [N_dep, 3]
        uav_points = sample['uav_points'].t().contiguous()  # shape: [N_uav, 3]
        edge_index = sample['dep_edge_index']               # shape: [2, E]

        # Normalize both point clouds to a common frame.
        dep_points_norm, uav_points_norm, center, scale = normalize_pair(dep_points, uav_points)
        return dep_points_norm, uav_points_norm, edge_index, center, scale


def variable_size_collate(batch):
    """
    Custom collate function to handle batches of samples with variable number of points.
    
    Each item in the batch is a tuple:
        (dep_points_norm, uav_points_norm, edge_index, center, scale)
    
    Returns:
        dep_list: List of dep_points_norm tensors
        uav_list: List of uav_points_norm tensors
        edge_list: List of edge_index tensors
        center_list: List of center tensors
        scale_list: List of scale values
    """
    dep_list, uav_list, edge_list = [], [], []
    center_list, scale_list = [], []
    for (dep_pts, uav_pts, e_idx, center, scale) in batch:
        dep_list.append(dep_pts)
        uav_list.append(uav_pts)
        edge_list.append(e_idx)
        center_list.append(center)
        scale_list.append(scale)
    return dep_list, uav_list, edge_list, center_list, scale_list

# ------------------------------
# Model Components
# ------------------------------

class FeatureExtractor(nn.Module):
    """
    Uses PointTransformerConv layers to extract per-point features from input positions.
    
    Expects:
        pos: Tensor of shape [N, 3] (point coordinates)
        edge_index: Tensor of shape [2, E] (precomputed edge indices)
        
    Returns:
        x_feat: Tensor of shape [N, feature_dim]
    """
    def __init__(self, feature_dim=64):
        super().__init__()
        # First PointTransformerConv layer: input dimension is 3 (point coordinates)
        self.pt_conv1 = PointTransformerConv(in_channels=3, out_channels=64)
        # Second layer: input dimension 64, output dimension is feature_dim
        self.pt_conv2 = PointTransformerConv(in_channels=64, out_channels=feature_dim)

    def forward(self, pos, edge_index):
        # Use the point positions as the initial feature vector.
        x_feat = self.pt_conv1(pos, pos, edge_index)
        x_feat = self.pt_conv2(x_feat, pos, edge_index)
        return x_feat


class FeatureExpansion(nn.Module):
    """
    Expands or truncates the feature matrix to a fixed number of points.
    
    Args:
        num_out_points: The desired number of output points.
    """
    def __init__(self, num_out_points=2048):
        super().__init__()
        self.num_out_points = num_out_points

    def forward(self, x_feat):
        """
        Args:
            x_feat: Tensor of shape [N_dep, feature_dim]
        Returns:
            x_expanded: Tensor of shape [num_out_points, feature_dim]
        """
        N_in, feat_dim = x_feat.shape
        if self.num_out_points <= N_in:
            x_expanded = x_feat[:self.num_out_points]
        else:
            repeats = (self.num_out_points // N_in) + 1
            x_tiled = x_feat.repeat((repeats, 1))
            x_expanded = x_tiled[:self.num_out_points]
        return x_expanded


class PointSetGenerator(nn.Module):
    """
    Generates new point coordinates from expanded features.
    """
    def __init__(self, feature_dim=64):
        super().__init__()
        self.mlp = nn.Sequential(
            nn.Linear(feature_dim, 64),
            nn.ReLU(),
            nn.Linear(64, 3)
        )

    def forward(self, x):
        # Returns predicted point coordinates of shape [num_out_points, 3]
        return self.mlp(x)


class SimplePointUpsampler(nn.Module):
    """
    A complete upsampling model that extracts features, expands them, and then generates
    an upsampled set of point coordinates.
    
    Args:
        feature_dim: Dimensionality of the learned point features.
        num_out_points: The number of output points to generate.
    """
    def __init__(self, feature_dim=64, num_out_points=2048):
        super().__init__()
        self.feature_extractor = FeatureExtractor(feature_dim=feature_dim)
        self.feature_expander = FeatureExpansion(num_out_points=num_out_points)
        self.point_generator = PointSetGenerator(feature_dim=feature_dim)

    def forward(self, dep_points, edge_index):
        """
        Args:
            dep_points: Tensor of shape [N_dep, 3] (input point coordinates)
            edge_index: Tensor of shape [2, E] (precomputed adjacency)
        Returns:
            pred_points: Tensor of shape [num_out_points, 3] (upsampled points)
        """
        # 1) Extract per-point features
        x_feat = self.feature_extractor(dep_points, edge_index)
        # 2) Expand or replicate features to match the desired output count
        x_expanded = self.feature_expander(x_feat)
        # 3) Generate new point coordinates from expanded features
        pred_points = self.point_generator(x_expanded)
        return pred_points


def chamfer_distance(pc1, pc2):
    """
    Computes the Chamfer Distance between two point clouds.
    
    Args:
        pc1: Tensor of shape [N1, 3]
        pc2: Tensor of shape [N2, 3]
    Returns:
        A scalar tensor representing the Chamfer distance.
    """
    # Compute pairwise distances between points in pc1 and pc2.
    dist = torch.cdist(pc1, pc2)  # shape: [N1, N2]
    # For each point in pc1, find the minimum distance to pc2
    min_dist_pc1, _ = dist.min(dim=1)
    # For each point in pc2, find the minimum distance to pc1
    min_dist_pc2, _ = dist.min(dim=0)
    return min_dist_pc1.mean() + min_dist_pc2.mean()



def hausdorff_distance(pc1, pc2):
    """Computes the bidirectional Hausdorff distance between two point clouds."""
    dist = torch.cdist(pc1, pc2)  # [N1, N2]
    min_dist_pc1, _ = dist.min(dim=1)  # [N1]
    min_dist_pc2, _ = dist.min(dim=0)  # [N2]
    return max(min_dist_pc1.max().item(), min_dist_pc2.max().item())






def weighted_chamfer_distance(pc1, pc2, weights):
    """
    Computes a weighted Chamfer Distance between two point clouds, using the same weights for both pc1 and pc2.
    
    Args:
        pc1: Tensor of shape [N1, 3]
        pc2: Tensor of shape [N2, 3]
        weights: Tensor of shape [N2] with weights for each point in pc2.
    
    Returns:
        A scalar tensor representing the weighted Chamfer distance.
    """
    # Compute pairwise distances between points in pc1 and pc2.
    dist = torch.cdist(pc1, pc2)  # shape: [N1, N2]
    
    # For each point in pc1, find the minimum distance to pc2.
    # We also retrieve the indices of the nearest neighbor in pc2.
    min_dist_pc1, nn_indices_pc1 = dist.min(dim=1)  # shape: [N1]
    
    # For each point in pc2, find the minimum distance to pc1.
    min_dist_pc2, _ = dist.min(dim=0)  # shape: [N2]
    
    # Use the weight of the corresponding nearest neighbor in pc2 for each point in pc1.
    weights_pc1 = weights[nn_indices_pc1]  # shape: [N1]
    
    # Compute the weighted loss for pc1.
    weighted_loss_pc1 = (min_dist_pc1 * weights_pc1).sum() / weights_pc1.sum()
    
    # Compute the weighted loss for pc2.
    weighted_loss_pc2 = (min_dist_pc2 * weights).sum() / weights.sum()
    
    return weighted_loss_pc1 + weighted_loss_pc2





import matplotlib.pyplot as plt






def run_inference_and_visualize(
    trained_model, 
    train_df, 
    index=0,  # Single index for one sample
    device='cuda', 
    width=14, 
    height=3,
    hide_labels=False,
    elev2=10,  # Elevation for rotated view
    azim2=120  # Azimuth for rotated view
):
    """
    Runs inference on a single sample, visualizes it from two different perspectives,
    and displays Chamfer and Hausdorff distances.
    """
    def process_sample(index):
        """Extracts, normalizes, runs inference, and computes distance metrics."""
        sample = train_df[index]
        dep_points_raw = sample['dep_points'].T  # [N_dep, 3]
        uav_points_raw = sample['uav_points'].T  # [N_uav, 3]
        edge_index = sample['dep_edge_index']

        # 1) Normalize the point clouds.
        dep_points_norm, uav_points_norm, center, scale = normalize_pair(dep_points_raw, uav_points_raw)

        # 2) Move to device.
        dep_points_norm = dep_points_norm.to(device)
        uav_points_norm = uav_points_norm.to(device)
        edge_index = edge_index.to(device)

        # 3) Build a PyG Data object for inference.
        data = Batch.from_data_list([Data(pos=dep_points_norm, edge_index=edge_index)])

        # 4) Run inference in normalized space.
        trained_model.eval()
        with torch.no_grad():
            # Now pass the single Data object.
            pred_points_norm = trained_model(data)  

        # 5) Compute distances (using your existing chamfer_distance and hausdorff_distance functions).
        orig_chamfer_dist = chmfr_dist(dep_points_norm, uav_points_norm)
        upsmpl_chamfer_dist = chmfr_dist(pred_points_norm, uav_points_norm)

        orig_hausdorff_dist = hausdorff_distance(dep_points_norm, uav_points_norm)
        upsmpl_hausdorff_dist = hausdorff_distance(pred_points_norm, uav_points_norm)
        print(f"CD (DEP vs UAV): {orig_chamfer_dist:.6f} | HD (DEP vs UAV): {orig_hausdorff_dist:.6f}")
        print(f"CD (Upsampled vs UAV): {upsmpl_chamfer_dist:.6f} | HD (Upsampled vs UAV): {upsmpl_hausdorff_dist:.6f}")

        # 6) Return CPU data for plotting.
        return (
            dep_points_norm.cpu(),
            uav_points_norm.cpu(),
            pred_points_norm.cpu(),
            orig_chamfer_dist,
            upsmpl_chamfer_dist,
            orig_hausdorff_dist,
            upsmpl_hausdorff_dist
        )

    # Process the sample at the given index.
    dep, uav, pred, chamfer_orig, chamfer_upsmpl, hausdorff_orig, hausdorff_upsmpl = process_sample(index)
    
    # Remove batch dimension from predicted points if it exists:
    if pred.ndim == 3 and pred.shape[0] == 1:
        pred = pred.squeeze(0)

    # Create subplots for visualization.
    fig, axes = plt.subplots(1, 7, figsize=(width, height), subplot_kw={'projection': '3d'})

    def configure_axes(ax, title, elev=20, azim=30):
        ax.view_init(elev=elev, azim=azim)
        if hide_labels:
            ax.set_xticks([])
            ax.set_yticks([])
            ax.set_zticks([])
            ax.set_title('')
        else:
            ax.set_title(title, fontsize=9)
            ax.tick_params(labelsize=6, pad=0)
            ax.ticklabel_format(style='plain', axis='both')
            ax.xaxis.get_offset_text().set_visible(False)
            ax.yaxis.get_offset_text().set_visible(False)
            ax.zaxis.get_offset_text().set_visible(False)

    def add_distance_labels(ax, x, y, chamfer_dist, hausdorff_dist, ttl):
        ax.text2D(x, y+0.2, f"{ttl}", transform=ax.transAxes, fontsize=7, color='black')
        ax.text2D(x, y+0.1, f"CD: {chamfer_dist:.4f}", transform=ax.transAxes, fontsize=7, color='black')
        ax.text2D(x, y, f"HD: {hausdorff_dist:.4f}", transform=ax.transAxes, fontsize=7, color='black')

    # Visualize the input and predictions.
    axes[0].scatter(dep[:, 0], dep[:, 1], dep[:, 2], c='blue', s=0.1, alpha=0.2)
    configure_axes(axes[0], "3DEP")
    
    axes[1].scatter(uav[:, 0], uav[:, 1], uav[:, 2], c='green', s=0.1, alpha=0.2)
    configure_axes(axes[1], "UAV")
    
    axes[2].scatter(pred[:, 0], pred[:, 1], pred[:, 2], c='red', s=0.1, alpha=0.2)
    configure_axes(axes[2], "Upsampled")
    
    axes[3].axis("off")
    add_distance_labels(axes[3], 0.3, 0.7, chamfer_upsmpl, hausdorff_upsmpl, "Upsampled vs UAV Dist")
    add_distance_labels(axes[3], 0.3, 0.3, chamfer_orig, hausdorff_orig, "3DEP vs UAV Dist")
    
    axes[4].scatter(dep[:, 0], dep[:, 1], dep[:, 2], c='blue', s=0.1, alpha=0.2)
    configure_axes(axes[4], "3DEP Rotated", elev=elev2, azim=azim2)
    
    axes[5].scatter(uav[:, 0], uav[:, 1], uav[:, 2], c='green', s=0.1, alpha=0.2)
    configure_axes(axes[5], "UAV Rotated", elev=elev2, azim=azim2)
    
    axes[6].scatter(pred[:, 0], pred[:, 1], pred[:, 2], c='red', s=0.1, alpha=0.2)
    configure_axes(axes[6], "Upsampled Rotated", elev=elev2, azim=azim2)
    
    plt.subplots_adjust(left=0, right=1, top=1, bottom=0, wspace=0)
    plt.show()
    
def run_inference_and_visualize_2plots(
    trained_model, 
    filtered_data, 
    index1=0, 
    index2=1, 
    device='cuda', 
    width=14, 
    height=3,
    hide_labels=False
):
    """
    Runs inference on two different samples (index1 & index2),
    visualizes both in a single row of six plots with an empty separator,
    and displays Chamfer Distance (CD) and Hausdorff Distance (HD).
    """

    def process_sample(index):
        """Extracts, normalizes, runs inference, and computes distance metrics."""
        sample = filtered_data[index]
        dep_points_raw = sample['dep_points'].T  # [N_dep, 3]
        uav_points_raw = sample['uav_points'].T  # [N_uav, 3]
        edge_index = sample['dep_edge_index']

        # 1) Normalize
        dep_points_norm, uav_points_norm, center, scale = normalize_pair(dep_points_raw, uav_points_raw)

        # 2) Move to device
        dep_points_norm = dep_points_norm.to(device)
        uav_points_norm = uav_points_norm.to(device)
        edge_index      = edge_index.to(device)

        # 3) Build a PyG Data object for inference.
        data = [Data(pos=dep_points_norm, edge_index=edge_index)]

        # 4) Run inference in normalized space.
        trained_model.eval()
        with torch.no_grad():
            # Now pass the single Data object.
            pred_points_norm = trained_model(data.pos, data.edge_index)


        # 5) Compute distances (using your existing chamfer_distance and hausdorff_distance functions).
        orig_chamfer_dist = chamfer_distance(dep_points_norm, uav_points_norm)
        upsmpl_chamfer_dist = chamfer_distance(pred_points_norm, uav_points_norm)

        orig_hausdorff_dist = hausdorff_distance(dep_points_norm, uav_points_norm)
        upsmpl_hausdorff_dist = hausdorff_distance(pred_points_norm, uav_points_norm)
        # 5) Return CPU data for plotting
        return (
            dep_points_norm.cpu(),
            uav_points_norm.cpu(),
            pred_points_norm.cpu(),
            orig_chamfer_dist,
            upsmpl_chamfer_dist,
            orig_hausdorff_dist,
            upsmpl_hausdorff_dist
        )

    # Process both samples
    dep1, uav1, pred1, chamfer1_orig, chamfer1_upsmpl, hausdorff1_orig, hausdorff1_upsmpl = process_sample(index1)
    dep2, uav2, pred2, chamfer2_orig, chamfer2_upsmpl, hausdorff2_orig, hausdorff2_upsmpl = process_sample(index2)
    
    # # Remove batch dimension from predicted points if it exists:
    # if pred1.ndim == 3 and pred1.shape[0] == 1:
    #     pred1 = pred1.squeeze(0)
    # if pred2.ndim == 3 and pred2.shape[0] == 1:
    #     pred2 = pred2.squeeze(0)
    # Create a single row of subplots with an empty separator
    fig, axes = plt.subplots(1, 7, figsize=(width, height), subplot_kw={'projection': '3d'})

    def configure_axes(ax, title):
        """Helper function to configure axis formatting."""
        if hide_labels:
            ax.set_xticks([])
            ax.set_yticks([])
            ax.set_zticks([])
            ax.set_title('')  # Remove title
        else:
            ax.set_title(title, fontsize=8)
            ax.tick_params(labelsize=6, pad=0)
            ax.ticklabel_format(style='plain', axis='both')
            ax.xaxis.get_offset_text().set_visible(False)
            ax.yaxis.get_offset_text().set_visible(False)
            ax.zaxis.get_offset_text().set_visible(False)

    def add_distance_labels(ax, chamfer_dist, hausdorff_dist):
        """Add Chamfer Distance (CD) and Hausdorff Distance (HD) labels to the plot."""
        ax.text2D(0, 0.1, f"CD: {chamfer_dist:.4f}", transform=ax.transAxes, fontsize=7, color='black')
        ax.text2D(0, 0, f"HD: {hausdorff_dist:.4f}", transform=ax.transAxes, fontsize=7, color='black')

    # First three plots => Sample index1
    axes[0].scatter(dep1[:, 0], dep1[:, 1], dep1[:, 2], c='blue', s=0.1, alpha=0.2)
    configure_axes(axes[0], f"3DEP ({index1})")
    add_distance_labels(axes[0], chamfer1_orig, hausdorff1_orig)

    axes[1].scatter(uav1[:, 0], uav1[:, 1], uav1[:, 2], c='green', s=0.1, alpha=0.2)
    configure_axes(axes[1], f"UAV ({index1})")

    axes[2].scatter(pred1[:, 0], pred1[:, 1], pred1[:, 2], c='red', s=0.1, alpha=0.2)
    configure_axes(axes[2], f"Upsampled ({index1})")
    add_distance_labels(axes[2], chamfer1_upsmpl, hausdorff1_upsmpl)

    # Empty plot for separation
    axes[3].axis("off")

    # Next three plots => Sample index2
    axes[4].scatter(dep2[:, 0], dep2[:, 1], dep2[:, 2], c='blue', s=0.1, alpha=0.2)
    configure_axes(axes[4], f"3DEP ({index2})")
    add_distance_labels(axes[4], chamfer2_orig, hausdorff2_orig)

    axes[5].scatter(uav2[:, 0], uav2[:, 1], uav2[:, 2], c='green', s=0.1, alpha=0.2)
    configure_axes(axes[5], f"UAV ({index2})")

    axes[6].scatter(pred2[:, 0], pred2[:, 1], pred2[:, 2], c='red', s=0.1, alpha=0.2)
    configure_axes(axes[6], f"Upsampled ({index2})")
    add_distance_labels(axes[6], chamfer2_upsmpl, hausdorff2_upsmpl)

    # Adjust layout to remove excess whitespace
    plt.subplots_adjust(left=0, right=1, top=1, bottom=0, wspace=0)

    plt.show()

if __name__ == "__main__":
    # Example usage when running this script directly
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = SimplePointUpsampler(feature_dim=96, num_out_points=4000).to(device)
    print("SimplePointUpsampler model instantiated.")


