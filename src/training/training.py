...
from process_pointcloud_stac import aggregate_grid
from preprocess import variable_size_collate, PointCloudUpsampleDataset,normalize_pair, check_tensor
from inference_eval import run_inference_and_visualize_2plots
from model import NodeShufflePointUpsampler_Relative_Attn, chamfer_distance_pytorch3d
...
def precompute_knn_inplace(model_data, k=10):
    """
    Loops through model_data and precomputes the KNN graph for each
    sample's 'dep_points'. Stores the result in sample['dep_edge_index'].

    model_data: list of dicts, each like:
      {
...
def create_dataloaders(train_dataset, val_dataset, k, batch_size):
    print(f"Precomputing k-NN with k={k} ...")
...
def process_batch(model, batch, device):
    ...
    for dep_points, uav_points, e_idx in zip(dep_list, uav_list, edge_list):
        ...
        loss = chamfer_distance_pytorch3d(pred_points, uav_points)
        if torch.isnan(loss):
...
# ---------------------------
# train_pointcloud_upsampler() Function
# ---------------------------
...
#         log_message = (f"Epoch {epoch+1}/{num_epochs} - Train Loss: {avg_train_loss:.6f}, "
#                        f"Val Loss: {avg_val_loss:.6f}, LR: {optimizer.param_groups[0]['lr']:.4e}, "
#                        f"Time: {epoch_time:.1f} sec")
#         print(log_message)
#         logger.info(log_message)
        
#         # If using Optuna, report the intermediate value and check for pruning.
...
#     summary_logger.info(f"Best Validation Loss: {best_val_loss:.4f}")
