...
def plot_naip_imagery(naip_data, naip_norm_stats=None, bbox_overlay=None, figsize=(15, 4), title=None):
    ...
    naip_data : dict
        ...
        - 'images': Tensor [n_images, 4, h, w] - NAIP imagery with 4 spectral bands
    ...
    naip_norm_stats : dict, optional
        ...
        - 'means': Tensor - Mean values for each band
        - 'stds': Tensor - Standard deviation values for each band
    ...
    for i, idx in enumerate(sorted_indices):
        ...
        # Denormalize image if normalization stats are provided
        if naip_norm_stats is not None and 'means' in naip_norm_stats and 'stds' in naip_norm_stats:
            means = naip_norm_stats['means']
            stds = naip_norm_stats['stds']
            
            # Convert to numpy if needed
            if isinstance(means, torch.Tensor):
                means = means.cpu().numpy() if means.is_cuda else means.numpy()
            if isinstance(stds, torch.Tensor):
                stds = stds.cpu().numpy() if stds.is_cuda else stds.numpy()
...
def generate_model_report(
    ...
    # Calculate input losses and improvement ratios for all samples
    for result in results:
        # Calculate input loss
        ...
        dep_length = torch.tensor([dep_points.shape[0]])
        uav_length = torch.tensor([uav_points.shape[0]])
        ...
        try:
            ...
            print(f"Error calculating input loss: {e}")
            input_loss = 1.0  # Default fallback
        
        # Replace any Inf loss values with 32000
        if np.isinf(result['loss']):
            print(f"Replacing Inf loss value for sample {result['tile_id']} with 32000")
            result['loss'] = 32000.0
        
        # Store input loss and calculate improvement ratio
